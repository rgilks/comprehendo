...
export class QuizRepository {
  ...
  findById = (id: number): Quiz | null => {
    try {
      const row = this.db.prepare('SELECT * FROM quiz WHERE id = ?').get(id) as QuizRow | undefined;
      if (!row) {
        return null;
      }
      ...
      try {
        ...
        console.error(
          `[QuizRepository] Failed to parse quiz content JSON for ID ${id}:`,
          jsonError
        );
    ...
    try {
      ...
      if (userId !== null) {
        const sql = `SELECT 
           ...
           LIMIT 1`;
        stmt = this.db.prepare<[number, string, string, string]>(sql);
        ...
        const sql = `SELECT id, language, level, content, created_at, question_language, user_id
           ...
           LIMIT 1`;
        stmt = this.db.prepare<[string, string, string]>(sql);
  ...
  countExercises = (passageLanguage: string, questionLanguage: string, level: string): number => {
    try {
...
// export const quizRepository = new QuizRepository(db);
