...
export class AdminRepository {
  ...
  getTableData = (tableName: string, page: number, limit: number): PaginatedTableData => {
    ...
    try {
      ...
      const allowedTableNames = this.getAllTableNames();
      ...
      const result = this.db.transaction(() => {
        ...
        const paginatedData = this.db.prepare(query).all(safeLimit, offset);
...
// Optional: Export a pre-configured instance if preferred over instantiation in actions
